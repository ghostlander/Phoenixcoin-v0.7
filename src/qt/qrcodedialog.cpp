...
QString QRCodeDialog::getURI() {
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PXC unit input in lnReqAmount, we generate the URI with PXC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CoinUnits::format(CoinUnits::PXC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
